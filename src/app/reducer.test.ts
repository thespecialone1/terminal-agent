...
describe('App Reducer', () => {
  ...
  test('ADD_CORE_MESSAGES appends messages', () => {
    ...
    const nextState = appReducer(initialState, { type: 'ADD_CORE_MESSAGES', payload: { sessionId, messages: [message] } });
    expect(nextState.sessions[0]!.messages).toHaveLength(1);
    expect(nextState.sessions[0]!.messages[0].content).toBe('hello');
  });

  test('CLEAR_MESSAGES empties the messages array', () => {
    const populatedState = { ...initialState, sessions: [{ ...initialState.sessions[0]!, messages: [{ role: 'user', content: 'test' } as any] }] };
    const nextState = appReducer(populatedState, { type: 'CLEAR_MESSAGES' });
    expect(nextState.sessions[0]!.messages).toHaveLength(0);
  });
});
